...
public class MainRestControllerAsync {
        ...
        if(customerService.validateToken(token)){
            ...
            if(request.getCookies()==null){
                ...
                inventoryAsyncResponse.subscribe((tempResponse) ->
                        ...
                        error ->
                        {
                            log.info("error processing the inventory response "+error.getMessage());
                ...
                if(tempCookie.isPresent()){
                    ...
                    } else if(updatedResponse.startsWith("Order-Service-Stage:QuantityCheckStage:QuantityCheckError")){
                        return ResponseEntity.ok("Order Id : "+orderId+" : Failed to process order because of internal error.");
